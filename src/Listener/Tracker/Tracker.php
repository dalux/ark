...
class Tracker
{
    ...
    function handle(array $data)
    {
        if ($data['event'] == 'event.framework.ready') {
            $debug = \Ark\Core\Captain::getInst()->config->global->debug;
            if ($debug && !\Ark\Core\Server::isCli()) {
                ob_start();
            }
        } elseif ($data['event'] == 'event.framework.shutdown') {
            //数据库执行语句
            $dbtrace = array(array('sql', 'time'));
            $_trace = \Ark\Core\Trace::get('database');
            $_trace || $_trace = array();
            foreach ($_trace as $k=> $v) { $dbtrace[] = $v; }
            //内存占用
            $memusage = \Ark\Core\Trace::get('memory');
            //页面执行完成总耗时
            $all_time = \Ark\Core\Timer::totalUsed();
            //数据连接时间
            $dbconnect_time = \Ark\Core\Timer::pick('db_connect_begin', 'db_connect_end');
            $dbconnect_time || $dbconnect_time = '0.0000';
            //引擎加载
            $driver_trace = array(array('name', 'value'));
            $drivers = \Ark\Core\Trace::get('driver');
            if (is_array($drivers)) {
            ...
            if (ob_get_length() > 0
                    && !\Ark\Core\Server::isCli()
                    && \Ark\Core\Captain::getInst()->config->global->debug) {
                ...
                if (strpos($user_agent, 'Chrome') !== false || strpos($user_agent, 'Firefox') !== false) {   //firefox下用firephp插件进行调试
                    ...
                    $usedinfo[] = array('memory', \Ark\Core\Toolkit::formatSize($memusage[1]-$memusage[0]));
                    $custom = array(array('name', 'value'));
                    $_custom = \Ark\Core\Trace::get('custom');
                    if ($_custom) {
            ...
            } elseif (\Ark\Core\Server::isCli()
                    && \Ark\Core\Captain::getInst()->request->get('debug')
                    && \Ark\Core\Captain::getInst()->config->global->debug) {
                ...
                echo "\t". \Ark\Core\Toolkit::formatSize($memusage[1]-$memusage[0]). PHP_EOL;
                echo PHP_EOL. '>>> Url Mode'. PHP_EOL;
                echo "\t". \Ark\Core\Captain::getInst()->router->getUrlModeName(). PHP_EOL;
