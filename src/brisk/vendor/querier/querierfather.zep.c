...
ZEPHIR_INIT_CLASS(Brisk_Vendor_Querier_QuerierFather) {
	...
	ZEPHIR_REGISTER_CLASS(Brisk\\Vendor\\Querier, QuerierFather, brisk, vendor_querier_querierfather, brisk_vendor_querier_querierfather_method_entry, ZEND_ACC_EXPLICIT_ABSTRACT_CLASS);

	zend_declare_property_string(brisk_vendor_querier_querierfather_ce, SL("_db_type"), "", ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(brisk_vendor_querier_querierfather_ce, SL("_parts"), ZEND_ACC_PROTECTED TSRMLS_CC);

	zend_declare_property_null(brisk_vendor_querier_querierfather_ce, SL("_db_bind"), ZEND_ACC_PROTECTED TSRMLS_CC);

	brisk_vendor_querier_querierfather_ce->create_object = zephir_init_properties_Brisk_Vendor_Querier_QuerierFather;
	return SUCCESS;
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, quote) {
	...
	zval *dbtype = NULL;
	zval *value, *dbtype_param = NULL, *type = NULL, *_1 = NULL, *key$$4 = NULL, *val$$4 = NULL, **_4$$4, *_5$$5 = NULL, *_7$$7, _8$$7, _9$$7, *_10$$8;
	...
	if (ZEPHIR_IS_STRING(type, "")) {
		...
		zephir_read_property_this(&type, this_ptr, SL("_db_type"), PH_NOISY_CC);
	}
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		zephir_is_iterable(value, &_3$$4, &_2$$4, 1, 0, "brisk/Vendor/Querier/QuerierFather.zep", 29);
		for (
		  ; zend_hash_get_current_data_ex(_3$$4, (void**) &_4$$4, &_2$$4) == SUCCESS
		  ; zend_hash_move_forward_ex(_3$$4, &_2$$4)
		) {
			ZEPHIR_GET_HMKEY(key$$4, _3$$4, _2$$4);
			ZEPHIR_GET_HVALUE(val$$4, _4$$4);
			ZEPHIR_CALL_METHOD(&_5$$5, this_ptr, "quote", &_6, 2, val$$4, type);
			zephir_check_call_status();
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, whereIn) {
	...
	if (zephir_is_instance_of(value, SL("Brisk\\Vendor\\Querier\\SelectBuilder") TSRMLS_CC)) {
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, whereExists) {
	...
	if (zephir_is_instance_of(query, SL("Brisk\\Vendor\\Querier\\SelectBuilder") TSRMLS_CC)) {
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, pickWherePart) {
	...
	zephir_array_fetch_string(&where_part, _0, SL("where"), PH_NOISY, "brisk/Vendor/Querier/QuerierFather.zep", 195 TSRMLS_CC);
	if (!(Z_TYPE_P(where_part) == IS_NULL)) {
		zephir_is_iterable(where_part, &_2$$3, &_1$$3, 0, 0, "brisk/Vendor/Querier/QuerierFather.zep", 216);
		for (
		  ...
		  ; zend_hash_move_forward_ex(_2$$3, &_1$$3)
			...
			zephir_array_fetch_string(&cond$$3, val$$3, SL("cond"), PH_NOISY, "brisk/Vendor/Querier/QuerierFather.zep", 201 TSRMLS_CC);
			ZEPHIR_OBS_NVAR(value$$3);
			zephir_array_fetch_string(&value$$3, val$$3, SL("val"), PH_NOISY, "brisk/Vendor/Querier/QuerierFather.zep", 202 TSRMLS_CC);
			if (Z_TYPE_P(value$$3) == IS_NULL) {
				ZEPHIR_CPY_WRT(part$$3, cond$$3);
				if (zephir_is_instance_of(cond$$3, SL("Brisk\\Vendor\\Querier\\BuildFather") TSRMLS_CC)) {
			...
			zephir_array_fetch_long(&_10$$4, where_part, (zephir_get_numberval(key$$3) + 1), PH_NOISY | PH_READONLY, "brisk/Vendor/Querier/QuerierFather.zep", 211 TSRMLS_CC);
			if (!(Z_TYPE_P(_10$$4) == IS_NULL)) {
				ZEPHIR_INIT_LNVAR(_11$$8);
				ZEPHIR_CONCAT_VS(_11$$8, part$$3, " AND ");
				ZEPHIR_CPY_WRT(part$$3, _11$$8);
			}
			zephir_array_append(&where, part$$3, PH_SEPARATE, "brisk/Vendor/Querier/QuerierFather.zep", 214);
		}
	}
	ZEPHIR_INIT_VAR(_12);
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, getRealSQL) {
	...
	zephir_check_call_status();
	zephir_is_iterable(bind, &_1, &_0, 0, 0, "brisk/Vendor/Querier/QuerierFather.zep", 241);
	for (
...
PHP_METHOD(Brisk_Vendor_Querier_QuerierFather, _parseExpr) {
	...
	if (ZEPHIR_GT_LONG(is_match, 0)) {
		zephir_array_fetch_long(&_1$$3, matches, 1, PH_NOISY | PH_READONLY, "brisk/Vendor/Querier/QuerierFather.zep", 265 TSRMLS_CC);
		ZEPHIR_CPY_WRT(matches, _1$$3);
		zephir_is_iterable(matches, &_3$$3, &_2$$3, 0, 0, "brisk/Vendor/Querier/QuerierFather.zep", 297);
		for (
		  ...
		  ; zend_hash_move_forward_ex(_3$$3, &_2$$3)
			...
			if (ZEPHIR_IS_STRING(v$$3, "?")) {
				...
				if (Z_TYPE_P(value) == IS_ARRAY) {
					...
					zephir_array_fetch(&val$$3, value, k$$3, PH_NOISY, "brisk/Vendor/Querier/QuerierFather.zep", 271 TSRMLS_CC);
				}
				...
				if (zephir_is_instance_of(val$$3, SL("Brisk\\Vendor\\Querier\\BuilderFather") TSRMLS_CC)) {
				...
				if (Z_TYPE_P(value) == IS_ARRAY) {
					ZEPHIR_OBS_NVAR(val$$3);
					zephir_array_fetch(&val$$3, value, k$$3, PH_NOISY, "brisk/Vendor/Querier/QuerierFather.zep", 284 TSRMLS_CC);
				}
