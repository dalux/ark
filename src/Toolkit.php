...
class Ark_Toolkit
{
    ...
    {
        if (!($dir = trim($dir))) {
            throw new Ark_Exception(Ark_Core::getInstance()->lang->get('tbox.invalid_create_path'));
        }
        $dirs = array($dir);
    ...
    static function strToXml($content)
    {
        ...
        if (!$data = simplexml_load_string($content, 'SimpleXMLElement', LIBXML_NOCDATA)) {
            ...
            libxml_clear_errors();
            throw new Exception(Ark_Core::getInstance()->lang->get('tbox.parse_string_failed', $error ? $error->message : '未知错误', $content));
        }
        return $data;
    }
