...
abstract class Ark_Sql_Builder_Father
{
    ...
    {
        if ($value instanceof Ark_Sql_Builder_Select) {
            $value = $value->getRealSQL();
            $expr = $field. ' IN (' . $value. ')';
        } elseif (is_array($value) && count($value) == 1) {
            $value = current($value);
            $expr = $field. '='. Ark_Sql_Builder::quote($value);
        } elseif (is_array($value)) {
            $value = Ark_Sql_Builder::quote($value);
            $expr = $field. ' IN ('. $value. ')';
        } else {
            $expr = $field. '='. Ark_Sql_Builder::quote($value);
        }
    ...
    {
        $expr = 'EXISTS (?)';
        if ($query instanceof Ark_Sql_Builder_Select) {
    ...
    function getRealSQL()
    {
        ...
        foreach ($bind as $key=> $val) {
            $sql = preg_replace('/'. $key. '/', Ark_Sql_Builder::quote($val), $sql, 1);
        }
        return $sql;
    }
