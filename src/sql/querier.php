...
class Ark_Sql_Querier
{
    ...
    {
        $insert = Ark_Sql_Builder::insert(self::$_conn->getDriverName())->into($this->_tb, $data);
    ...
    {
        $update = Ark_Sql_Builder::update(self::$_conn->getDriverName())->set($this->_tb, $data);
    ...
    {
        $delete = Ark_Sql_Builder::delete(self::$_conn->getDriverName())->from($this->_tb);
        foreach ($condition as $k=> $v) {
            if (is_array($v)) {
                $kk = strtoupper(current(array_keys($v)));
                $vv = current($v);
    ...
    {
        $select = Ark_Sql_Builder::select(self::$_conn->getDriverName())->from($this->_tb, $fields);
        foreach ($condition as $k=> $v) {
            if (is_array($v)) {
                $kk = strtoupper(current(array_keys($v)));
                $vv = current($v);
    ...
    {
        $select = Ark_Sql_Builder::select(self::$_conn->getDriverName())->from($this->_tb, $fields)->limit($count, $offset);
