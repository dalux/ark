...
class Ark_Event
{
    ...
    static function onListening($event, array $data = array())
    {
        ...
        foreach ($listener as $name=> $item) {
            ...
            $data = $item($data);
            if (!is_array($data)) {
                throw new Exception(Ark_Core::getInstance()->lang->get('event.data_must_array', $event));
            }
            $now_keys = array_keys($data);
            if ($diff = array_diff($ori_keys, $now_keys)) {
                throw new Exception(Ark_Core::getInstance()->lang->get('event.data_key_must_equal', $event));
            }
        }
        return $data;
    }
