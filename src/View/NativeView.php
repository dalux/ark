...
class NativeView extends ViewFather
{
    ...
    {
        $config = Kernel::getInstance()->config->view;
    ...
    function display($template, $return = false)
    {
        ...
        if (!is_file($template)) {
            throw new ViewException(Language::get('view.template_not_found', Loader::reducePath($template)));
        }
        $data = array(
            'template'=> $template,
            'driver'=> 'Native',
    ...
    function import($path, $params = array(), $return = false)
    {
        ...
        if (!is_file($template)) {
            throw new ViewException(Language::get('view.include_file_not_found', Loader::reducePath($template)));
        }
        extract($this->_storage, EXTR_REFS);
