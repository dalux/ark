...
class Database
{
    ...
    {
        $config = Ark_Core::getInstance()->config->database->$name;
        if (!$config) {
            throw new Exception(Ark_Core::getInstance()->lang->get('db.config_not_found', $name));
        }
        ...
        if (!Loader::findClass($driver)) {
            throw new Exception(Ark_Core::getInstance()->lang->get('db.driver_not_found', $driver));
        }
        $instance = new $driver($dsn, $option);
        if (!$instance instanceof DbInterface) {
            throw new Exception(Ark_Core::getInstance()->lang->get('db.driver_implement_error', $driver, '\\Ark\\Contract\\Database'));
        }
    ...
    static function parseConnectUrl($url)
    {
        if (!$parsed = parse_url($url)) {
            throw new Exception(Ark_Core::getInstance()->lang->get('db.invalid_conn_string', $url));
        }
        $config = array(
            'type'=> $parsed['scheme'],
            'host'=> $parsed['host'],
