...
class Ark_Router_Adapter
{
    ...
    static function getDriver()
    {
        ...
        if (!$driver = $config['driver']) {
            throw new Exception(Ark_Language::get('router.invalid_driver_name'));
        } elseif (!Ark_Loader::findClass($driver)) {
            throw new Ark_Router_Exception(Ark_Language::get('router.driver_not_found', $driver));
        }
        $instance = new $driver();
        if (!$instance instanceof Ark_Router_Contract) {
            throw new Ark_Router_Exception(Ark_Language::get('router.driver_implement_error', $driver, 'Ark_Router_Contract'));
        }
