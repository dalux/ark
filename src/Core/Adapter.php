...
class Adapter
{
    ...
    {
        $config = Engine::getInst()->config->cache;
    ...
    {
        $config = Engine::getInst()->config->database;
        if (!$config || !$config = $config->$name) {
    ...
    {
        $config = Engine::getInst()->config->router->toArray();
    ...
    {
        $config = Engine::getInst()->config->session->toArray();
        if (!$driver = $config['driver']) {
    ...
    {
        $config = Engine::getInst()->config->view->toArray();
        if (!$driver = $config['driver']) {
