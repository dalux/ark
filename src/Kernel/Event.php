...
class Event
{
     ...
     * @return null
     */
    public static function add($event, callable $listener)
    {
     ...
     * @return array
     */
    public static function trigger($event, array $data = [])
    {
        if (!isset(self::$_listener[$event])) {
            return $data;
        }
        ...
        foreach ($listener as $key=> $item) {
            ...
            if (!is_array($result)) {
                throw new EventException(Language::get('event.data_must_array', $event));
            }
            $now_keys = array_keys($result);
			$diff = array_diff($ori_keys, $now_keys);
            if (count($diff) > 0) {
                throw new EventException(Language::get('event.data_key_must_equal', $event));
            }
        }
        return $result;
    }
