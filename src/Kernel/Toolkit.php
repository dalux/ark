            ...
            foreach ($item as $k=> $v) {
                ...
                if ($k == 'required' && $v) {
                    if (is_string($v)) {
                        ...
                        if (!$pass) {
                            throw new RuntimeException(Language::get('tool.validator_field_missing', $field));
                        }
                ...
                if (!is_null($value)) {
                    ...
                    if ($k == 'type') {
                        ...
                        if ($v != $type) {
                            throw new RuntimeException(Language::get('tool.validator_field_errtype', $field, $v, $type));
                        }
                    }
                    //范围校验
                    if ($k == 'range' && is_array($v)) {
                        if (!in_array($value, $v)) {
                            throw new RuntimeException(Language::get('tool.validator_field_outrange', $field));
                        }
                    }
                    //最小值校验
                    if ($k == 'min_value' && $value < $v) {
                        throw new RuntimeException(Language::get('tool.validator_filed_lessthan', $field, $v));
                    }
                    //最大值校验
                    if ($k == 'max_value' && $value > $v) {
                        throw new RuntimeException(Language::get('tool.validator_filed_morethan', $field, $v));
                    }
                    //最小长度校验
                    if ($k == 'min_length' && is_string($value) && strlen($value) < $v) {
                        throw new RuntimeException(Language::get('tool.validator_filed_minlength', $field, $v, strlen($value)));
                    }
                    //最大长度校验
                    if ($k == 'max_length' && is_string($value) && strlen($value) > $v) {
                        throw new RuntimeException(Language::get('tool.validator_filed_maxlength', $field, $v, strlen($value)));
                    }
                    //是否相等校验
                    if ($k == 'equal') {
                        if (is_string($value) && $value != $v) {
                            throw new RuntimeException(Language::get('tool.validator_filed_noequal', $field, $v, $value));
                        } elseif (is_object($value) && !$value instanceof $v) {
                            throw new RuntimeException(Language::get('tool.validator_filed_errobject', $field, get_class($v)));
                        }
                    }
                    //是否匹配规则
                    if ($k == 'match' && !preg_match($v, $value)) {
                        throw new RuntimeException(Language::get('tool.validator_filed_nomatch', $field, $v));
                    }
                }
            }
        }
