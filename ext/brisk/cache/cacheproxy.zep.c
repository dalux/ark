...
ZEPHIR_INIT_CLASS(Brisk_Cache_CacheProxy) {
	...
	ZEPHIR_REGISTER_CLASS(Brisk\\Cache, CacheProxy, brisk, cache_cacheproxy, brisk_cache_cacheproxy_method_entry, 0);

	zend_declare_property_null(brisk_cache_cacheproxy_ce, SL("_cache"), ZEND_ACC_PRIVATE TSRMLS_CC);
...
PHP_METHOD(Brisk_Cache_CacheProxy, doProxy) {
	...
	if (!(zephir_is_callable(_0 TSRMLS_CC))) {
		...
		zephir_check_call_status();
		zephir_throw_exception_debug(_1$$3, "brisk/Cache/CacheProxy.zep", 37 TSRMLS_CC);
		ZEPHIR_MM_RESTORE();
		return;
	}
	ZEPHIR_CPY_WRT(key, name);
	if (ZEPHIR_IS_STRING(name, "")) {
		ZEPHIR_CALL_METHOD(&key, this_ptr, "_name", NULL, 24, target, method, args);
		zephir_check_call_status();
	}
...
PHP_METHOD(Brisk_Cache_CacheProxy, _name) {
	...
	ZEPHIR_CALL_FUNCTION(&_1, "serialize", NULL, 25, args);
	zephir_check_call_status();
	ZEPHIR_INIT_VAR(_2);
