...
ZEPHIR_INIT_CLASS(Brisk_Sql_Delete_Oci) {

	ZEPHIR_REGISTER_CLASS_EX(Brisk\\Sql\\Delete, Oci, brisk, sql_delete_oci, brisk_sql_delete_ce, brisk_sql_delete_oci_method_entry, 0);

	zend_declare_property_string(brisk_sql_delete_oci_ce, SL("_db_type"), "oci", ZEND_ACC_PROTECTED TSRMLS_CC);
 ...
 * @return string
 */
PHP_METHOD(Brisk_Sql_Delete_Oci, quote) {
	...
	if (Z_TYPE_P(value) == IS_ARRAY) {
		zephir_is_iterable(value, &_3$$3, &_2$$3, 1, 0, "brisk/Sql/Delete/Oci.zep", 23);
		for (
		  ...
		  ; zend_hash_move_forward_ex(_3$$3, &_2$$3)
			...
			ZEPHIR_CALL_METHOD(&_5$$4, this_ptr, "quote", &_6, 125, val$$3);
			zephir_check_call_status();
